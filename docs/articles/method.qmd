---
title: "The method: identification and two-step Bayesian inference"
subtitle: "What scspill estimates, and exactly how"
---

This page states the model and the estimator implemented by scspill —
Sakaguchi & Tagawa's *Identification and Bayesian Inference for Synthetic
Control Methods with Spillover Effects* — and documents precisely where this
...
## Where scspill departs from the R replication package

scspill defaults to the *paper-correct* behavior and documents every
...
| 5 | the factor block's $\omega_k$ conditional treats $\omega_k$ as a variance multiplier while its $\boldsymbol\eta$ / $\sigma_\eta^2$ conditionals treat it as a precision — mutually inconsistent conditionals | the paper's parametrization $\eta_{ik} \sim \mathcal N(0, \sigma^2_\eta \omega_k)$ throughout | — (the R combination is not a valid Gibbs sampler for any posterior) |
...
test](validation.qmd) — with the fixes, every sampler block passes it.
