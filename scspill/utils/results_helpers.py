...
metrics through the same pydantic models, computed from one consistent source
so ATT / %ATT / gap / pre+post RMSE / R-squared are derived identically.
Adapted from ``mlsynth.utils.results_helpers``.
"""
...
def normalize_counterfactual_band(
    ...
    *,
    ...
    NaN where the band does not reach, or ``(None, None)`` when no band is
    given. Bounds come in pairs; a one-sided band or a length mismatch is a
    malformed band (:class:`~scspill.exceptions.ScspillConfigError`). When
    ``periods`` is given the band aligns by matching those labels into
    ``time_periods``; otherwise a full-length band is used as-is.
    """
    ...
    if lo.size != hi.size:
        raise ScspillConfigError(
            f"Prediction-interval 'lower' and 'upper' differ in length ({lo.size} vs {hi.size})."
        )
    ...
    if band_p is not None and band_p.size == lo.size:
        ...
        for p, lo_val, hi_val in zip(band_p.tolist(), lo, hi, strict=True):
            if p not in index:
                raise ScspillConfigError(
                    f"Band period {p!r} is not in the counterfactual time axis."
                )
            full_lo[index[p]] = lo_val
            full_hi[index[p]] = hi_val
    elif lo.size == n:
        full_lo[:] = lo
        full_hi[:] = hi
    else:
...
def make_weights_results(
    ...
    extra : dict, optional
        Additional fields merged into ``summary_stats``.
    """
    ...
    w = np.array(list(donor_weights.values()), dtype=float) if donor_weights else np.array([])
...
def effect_metrics(
    ...
    The single source of truth for the series-derived quantities every effect
    estimator reports, so they are computed identically everywhere: ATT (mean
    post-period gap), percent ATT, the per-period gap, pre- and post-period
    RMSE, and pre-period R-squared.
...
def build_effect_submodels(
    ...
    *,
    ...
    att_std_err: float | None = None,
    effects_overrides: dict[str, Any] | None = None,
    fit_overrides: dict[str, Any] | None = None,
    additional_effects: dict[str, Any] | None = None,
    additional_metrics: dict[str, Any] | None = None,
    intervention_time: Any | None = None,
    prediction_interval: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Build the standardized effect-result sub-models from two outcome paths.
    ...
    computes authoritatively (e.g. a method-specific ATT) instead of the
    canonical one, so validated numbers never drift.
    """
    ...
    if prediction_interval:
        axis = np.asarray(time_periods) if time_periods is not None else np.arange(obs.size)
        pi = dict(prediction_interval)
        lo, hi = normalize_counterfactual_band(
            pi.get("lower"), pi.get("upper"), time_periods=axis, periods=pi.get("periods")
        )
        lo_s, hi_s = normalize_counterfactual_band(
            pi.get("lower_simultaneous"),
            pi.get("upper_simultaneous"),
            time_periods=axis,
            periods=pi.get("periods"),
        )
        ts_kwargs.update(
            counterfactual_lower=lo,
            counterfactual_upper=hi,
